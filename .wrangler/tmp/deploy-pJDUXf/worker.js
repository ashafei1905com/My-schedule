  ...
  async fetch(request, env) {
    ...
    if (request.method === "POST" && url.pathname === "/api/build-schedule") {
      try {
        ...
        const messyText = (body2.messyText || "").trim();
        const today = body2.today || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
        const lang = body2.lang === "ar" ? "ar" : "en";
        if (!messyText) {
          return Response.json({ error: "messyText is required" }, { status: 400 });
        }
        const system2 = `You are an expert AI Scheduling Agent. Turn a messy task list into a realistic daily schedule.
RULES: 15-min buffers between tasks; high-focus in the morning; every block has start, end, emoji.
Return ONLY JSON:
{"date":"YYYY-MM-DD","blocks":[{"start":"HH:MM","end":"HH:MM","task":"string","emoji":"\u{1F4DA}","category":"Deep Work","buffer_after_min":15}],"pro_tip":"short tip"}`;
...
__name(usdaRelevanceScoreDetailed, "usdaRelevanceScoreDetailed");
var USDA_RELEVANCE_MIN_SCORE = 0.5;
var USDA_RELEVANCE_MIN_OVERLAP_COUNT = 2;
function usdaPickBestResult(foods, query, isIngredientLookup) {
  if (!Array.isArray(foods) || !foods.length) return null;
  const qTokenCount = usdaNormalizeForScore(query).filter((w) => w.length >= 3).length;
  const scored = foods.map((f) => ({ food: f, ...usdaRelevanceScoreDetailed(query, f.description) })).filter((x) => {
...
async function usdaSearchRaw(env, query, isIngredientLookup) {
...
async function usdaNormalizeAndRetry(env, originalQuery, isIngredientLookup) {
  if (!env.GROQ_API_KEY) return null;
  ...
  if (needsDecomposition(query)) {
    ...
    if (decomposed) {
      ...
      for (const ing of decomposed.ingredients) {
        ...
        const cp = ingResult.per100g.p * scale2, cc = ingResult.per100g.c * scale2, cf = ingResult.per100g.f * scale2, cb = (ingResult.per100g.b || 0) * scale2, ck = ingResult.per100g.k * scale2;
        total.p += cp;
        total.c += cc;
        total.f += cf;
        total.b += cb;
    ...
    for (let qi = 0; qi < queries.length; qi++) {
      ...
      for (const food of foods) {
        ...
        items.push({
          ...
          components: food.components || void 0
        });
        anyMatched = true;
