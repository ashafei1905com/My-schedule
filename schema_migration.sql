...
-- Belt-and-suspenders against the "same reminder fires repeatedly" bug: even if the
-- Worker's own dedup guard (INSERT ... WHERE NOT EXISTS) is ever bypassed by a future
-- code change, this constraint makes it physically impossible for two live rows to
-- exist for the same (user, task, reminder kind, day) — the second INSERT simply
-- fails at the DB level instead of silently creating a duplicate.
CREATE UNIQUE INDEX IF NOT EXISTS idx_reminders_dedup
  ON scheduled_reminders(user_uid, task_id, reminder_type, fire_date);
