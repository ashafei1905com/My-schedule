import type { Category, DaySchedule, Macros, Task, WeekSchedule } from "./types";
...
function sleepTask(id: string, start: string): Task {
  return task({
    id,
    start,
...
export function buildFocusWeek(opts: TemplateOpts): WeekSchedule {
  ...
  for (let jd = 0; jd < 7; jd++) {
    ...
    if (!friday) {
      ...
      tasks.push(
        deepWork(`${jd}-dw2`, bufferAfter(mid, 10), peakEnd, ["Deep work II", "شغل عميق ٢"]),
      );
    } else {
      tasks.push(
    ...
    tasks.push(sleepTask(`${jd}-sl`, bed));
...
export const CATEGORY_META: Record<Category, { token: string; icon: string }> = {
  prayer: { token: "cat-prayer", icon: "Moon" },
  food: { token: "cat-food", icon: "Utensils" },
  gym: { token: "cat-gym", icon: "Dumbbell" },
  swim: { token: "cat-swim", icon: "Waves" },
  recovery: { token: "cat-recovery", icon: "HeartPulse" },
  quran: { token: "cat-quran", icon: "BookOpen" },
  prog: { token: "cat-prog", icon: "Code" },
  sleep: { token: "cat-sleep", icon: "MoonStar" },
  free: { token: "cat-free", icon: "Coffee" },
  snack: { token: "cat-snack", icon: "Apple" },
  sunrise: { token: "cat-sunrise", icon: "Sunrise" },
  work: { token: "cat-prog", icon: "Briefcase" },
  admin: { token: "cat-free", icon: "Inbox" },
};
